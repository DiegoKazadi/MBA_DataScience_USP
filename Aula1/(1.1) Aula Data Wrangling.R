...
# Poderia ser utilizada para criar informações mais espec?ficas sobre o dataset
...
nova_base_grupos <- nova_base %>% group_by(periodo, perfil) %>% 
  summarise(tempo_m?dio = mean(tempo),
            m?nimo = min(tempo),
            m?ximo = max(tempo),
            contagem = n()) %>% 
  arrange(desc(m?ximo))
...
atualizar_teto <- function(historico) {
  atual <- ((historico + 17)/2)
  if (atual <= 100) {
    return(atual)}
  else {
...
m?dias(nova_base$tempo)
m?dias(nova_base$distancia)
m?dias(nova_base$semaforos)
...
m?dias_var <- list(5, 10, 15)
desv_pad_var <- list(1, 2, 3)

map2(m?dias_var, desv_pad_var, rnorm, n = 5)
...
parametros <- list(tamanho_var, m?dias_var, desv_pad_var) # sequência da fórmula

pmap(parametros, rnorm)

# Na prática, para evitar erros, é melhor nomear os argumentos

parametros2 <- list(mean = m?dias_var, sd = desv_pad_var, n = tamanho_var)
